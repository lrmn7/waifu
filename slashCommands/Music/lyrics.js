    ...
    if (Sub === 'current') {
      try {
        if (!client.usernews.get(guild.id, 'news').includes(interaction.user.id)) {
          ...
          if (!queue) {
            interaction.reply({ content: `${client.emoji.cross} | There is nothing in the queue right now!`, ephemeral: true }).then(() => {
              interaction.followUp({ content: `<@${interaction.user.id}>`, embeds: [new EmbedBuilder().setColor(client.important.MAIN_COLOR).setTitle(client.config.alert.title).setDescription(client.config.alert.desc).setThumbnail(client.config.alert.thumb)] })
            })
          }
          ...
          try {
            ...
            if (!lyrics) {
              await interaction.followUp({ content: `${client.emoji.cross} | I couldn't find any lyrics for that song!`, ephemeral: true }).then(() => {
                interaction.followUp({ content: `<@${interaction.user.id}>`, embeds: [new EmbedBuilder().setColor(client.important.MAIN_COLOR).setTitle(client.config.alert.title).setDescription(client.config.alert.desc).setThumbnail(client.config.alert.thumb)] })
              })
            }
          } catch (e) {
    ...
    if (Sub === 'song') {
      try {
        if (!client.usernews.get(guild.id, 'news').includes(interaction.user.id)) {
          ...
          const lyricsEmbed2 = new EmbedBuilder()
            .setColor(client.important.MAIN_COLOR)
            .setTitle(`${string}`)
            .setDescription(`${lyrics || 'No Lyrics Found!'}`)
            .setFooter({ text: `Requested by ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
