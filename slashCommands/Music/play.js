    ...
    try {
      if (!client.usernews.get(guild.id, "news").includes(interaction.user.id)) {
        if (interaction.options.getString('input')) {
          ...
          if (string == "https://www.youtube.com/watch?v=xvFZjo5PgG0") return interaction.reply({ content: `${client.emoji.warning} | Oops! this song/link is not allowed`, ephemeral: true }).then(() => {
        ...
        if (interaction.options.getString('input')) {
          const string = interaction.options.getString('input');
          if (string == "https://www.youtube.com/watch?v=xvFZjo5PgG0") return interaction.reply({ content: `${client.emoji.warning} | Oops! this song/link is not allowed`, ephemeral: true })
          if (!interaction.member.voice.channel) {
