  ...
  execute: async (client, interaction) => {
    ...
    try {
      if (!client.usernews.get(guild.id, 'news').includes(interaction.user.id)) {
        let link = 'https://open.spotify.com/playlist/6R4aE0V0YmcKJH8qjYBLFK?si=a751cb69805e4d21'
        if (args[0]) {
        ...
        let link = 'https://open.spotify.com/playlist/2miIpbwlt1bLuQKdpKhQMK?si=670ad35f4e2e4566'
        if (args[0]) {
          // ncs | no copyrighted music
          if (args[0].toLowerCase().startsWith('n')) link = 'https://open.spotify.com/playlist/7sZbq8QGyMnhKPcLJvCUFD'
          // pop
